...
public class PrintTest extends JFrame
{
	...
	private class RootNode extends JPanel
	{
		RootNode()
		{
			...
			JButton pss = new JButton("Service UI");
			add( pss);
    ...
    {
    	DocFlavor dv = DocFlavor.STRING.TEXT_HTML;
		...
		SimpleDoc simpleDoc = new SimpleDoc(sb.toString(), dv, null);
		try
		{
			j.print( simpleDoc, null );
		} catch (Exception e)
		{
...
//    	}
    ...
    private void showPageSetup(final ActionEvent event) {
		PrintService printService = PrintServiceLookup.lookupDefaultPrintService();
		ServiceUIFactory serviceUIFactory = printService.getServiceUIFactory();
		Object bdialog = serviceUIFactory.getUI( ServiceUIFactory.ADMIN_UIROLE, ServiceUIFactory.JDIALOG_UI);
		JDialog jDialog = (JDialog)bdialog;
